<script>
	...
	export default {
		...
		methods: {
			...mapActions(['setNewPin']),
			async handleInput(code) {
				try {
					this.pin += code
					let pinLength = this.pin.length;
					if (pinLength >= 6) {
						this.setNewPin(this.pin)
						uni.redirectTo({
							url: '/pages/verifyResetPwd/verifyResetPwd'
						})
					}
				} catch (error) {
					console.error(error)
	...
	<view class="page-container">
		...
		<view class="page-body" :style="{paddingTop: navHeight + 'px'}">
			...
			<view class="sub-title">
				Your mnemonic phrase has been successfully verified. You may now create a new password.
			</view>
